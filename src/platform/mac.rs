...
/// The cached native menu metrics (computed once from their no-popup sources).
fn native_menu_metrics() -> NativeMenuMetrics {
    ...
    *CACHE.get_or_init(|| NativeMenuMetrics {
        // Version-gated: 12pt on Tahoe (macOS 26+), 6pt pre-Tahoe, 0 on Catalina-.
        corner_radius: read_system_corner_radius(),
        // The native `NSMenu` text inset (14pt); a stable preset value.
        leading_inset: crate::theme::MACOS_LEADING_INSET,
        // Prefers a no-popup `NSMenu.size` read (never displayed; ~24pt on Tahoe),
        // falling back to the ratio at the ~13pt native menu font size off-main.
